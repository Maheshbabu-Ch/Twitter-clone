...
export default function Profile() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const userResponse = await axios.get(`user/find/${id}`);
  ...
  const getTweets = async () => {
    try {
      // console.log(user)
      const response = await axios.get(`/tweets/user/${user._id}`);
        ...
        tweetsData.map(async (tweet) => {
          ...
          const handlelike = async(e)=> {
              ...
              {   if(localStorage.getItem('userID')!==null){
                  const like = await axios.put(`http://localhost:5000/api/tweets/${tweet._id}/like`,
      ...
      // console.error(error);
        ...
        const followhandler = async () => {
            try {
              if (currentUser && currentUser.following && user && user._id) {
                if (currentUser.following.includes(user._id)) {
                  await axios.put(`http://localhost:5000/api/user/unfollow/${user._id}`, null, {
                    headers: {
                      'x-token': localStorage.getItem('token'),
                    },
                  });
                } else {
                  await axios.put(`http://localhost:5000/api/user/follow/${user._id}`, null, {
                    headers: {
                      'x-token': localStorage.getItem('token'),
                    },
                  });
                ...
                const updatedCurrentUserResponse = await axios.get(`http://localhost:5000/api/user/find/${localStorage.getItem('userID')}`);
                const updatedUserResponse = await axios.get(`http://localhost:5000/api/user/find/${id}`);
          ...
          const deletehandler = async() => {
            try{
                  await axios.delete(`http://localhost:5000/api/user/${localStorage.getItem('userID')}`,{
                    headers: {
                      'x-token': localStorage.getItem('token'),
                    },
                  });
